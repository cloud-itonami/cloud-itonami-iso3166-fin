...
2. Register the client's intake: business type, target public function,
   prior filing history in Finland if any.
3. Run the advisor in read-only mode against Hilma
   (hankintailmoitukset.fi).
4. Compare the checklist against the client's current documentation
   (PRH Trade Register status).
...
- named referral relationship with Finnish-licensed counsel or a
...
a working referral relationship with Finnish-licensed counsel or a
registered agent for whatever licensed representation the law of
Finland requires for actual public-procurement filings.
