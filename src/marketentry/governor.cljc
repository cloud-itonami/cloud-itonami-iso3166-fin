(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Finland-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Finland-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-eu-establishment?
                                       true` (near-universal for Finland
                                       ...
                                       Grounded in EU establishment or Finnish
                                       business ID for many procedures
                                       (contracting authorities / Hansel).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(YTUNNUS/統一資格/Hilma登録/代理人確認等)が充足していない状態での提案"}]))))
...
(defn- eu-establishment-missing-violations
  ...
  `:requires-eu-establishment?` ground truth (most Finland public
    ...
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-eu-establishment? e))
        ...
        [{:rule :eu-establishment-missing
          :detail (str subject " はフィンランド居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- ytunnus-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-ytunnus? true`, INDEPENDENTLY check
  `:ytunnus-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-ytunnus? e))
                 (not (true? (:ytunnus-verified? e))))
        [{:rule :ytunnus-unverified
          :detail (str subject " はYTUNNUS確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (ytunnus-unverified-violations request st)
