# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Finland

## Classification

- Repository: `cloud-itonami-iso3166-fin`
- ISO 3166: `FIN` (Finland)
...
- an already-incorporated `cloud-itonami-cofog-{code}` /
  ...
  `cloud-itonami-{ISIC}` operator wanting to bid on a Finnish
  public contract
...
- registration walkthrough for Hilma (hankintailmoitukset.fi), Finland's
  official public-procurement notification channel operated by the
  Ministry of Finance, mandatory for all contracting authorities above
  the national thresholds set by the Public Procurement Act (Hankintalaki
  1397/2016) — publicly searchable, no account required
- business/tax registration checklist: a Kaupparekisteriote (Trade
  Register extract) from the PRH (Finnish Patent and Registration
  Office), typically required at bid submission
- local-content / preferential-procurement navigation: EU-wide open
  ...
  member state), but Finnish-language submission is typically required
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Finnish-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Finland).
